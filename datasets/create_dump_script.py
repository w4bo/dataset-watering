...
with open('dump-clean.sh', 'w') as f:
    ...
    f.write('expdp {user}/{pwd}@{host}:{port}/{db} directory=oracle_dump dumpfile={user}_$T.dmp SCHEMAS={user}\n'.format(user=config['OUT_ORA1_USER'], pwd=config['OUT_ORA1_PWD'], port=config['OUT_ORA1_PORT'], db=config['OUT_ORA1_DB']))
    f.write('expdp {user}/{pwd}@{host}:{port}/{db} directory=oracle_dump dumpfile={user}_$T.dmp SCHEMAS={user}\n'.format(user=config['OUT_ORA2_USER'], pwd=config['OUT_ORA2_PWD'], port=config['OUT_ORA2_PORT'], db=config['OUT_ORA2_DB']))
